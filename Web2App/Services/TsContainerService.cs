...
namespace Web2App.Services
{
    public class TsContainerService : ITsContainerService
    {
        ...
        public async Task<TSContainer> MakeTsContainer(QrCodePostModel model)
        {
            ...
            //Create Proto Info
            var protoInfo = new ProtoInfo("web2app", model.DataUrl == null ? "1.0" : "1.1");
            ...
            dataInfo.DataURI = model.DataUrl;
            //Finger Print

            var clientInfo = new ClientInfo(model.ClientId, model.IconUri, model.CallBackUrl, model.HostName);
            ...
            if (model.DataUrl == null)
            {
                var jobject = JObject.Parse(signableJsonContainer);

                jobject.Properties()
                        .Where(attr => attr.Name.StartsWith("DataInfo"))
                        .ToList()
                        .ForEach(attr => attr.Remove());
                signableJsonContainer = Regex.Replace(jobject.ToString(), "(\"(?:[^\"\\\\]|\\\\.)*\")|\\s+", "$1");
